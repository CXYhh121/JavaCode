...
<script  language="javascript" >
    function top2(){
        form3.action="<%=basePath%>szy.jsp?page=1";
        form3.submit();
    }
    function last2(){
        if(form3.pageCount.value==0){//如果总页数为0，那么最后一页为1，也就是第一页，而不是第0页
            ...
            form3.action="<%=basePath%>szy.jsp?page="+form3.pageCount.value;
            form3.submit();
        }
    }
    function pre2(){
        ...
        if(page<=1){
            ...
            form3.action="<%=basePath%>szy.jsp?page="+(page-1);
            form3.submit();
        }
    }
    ...
    function bjump2(){
        ...
        if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
            if(pageCount==0){
                ...
                form3.action="<%=basePath%>szy.jsp?page="+pageCount;
                form3.submit();
            }
        }
            ...
            if(page==0){
                ...
                form3.action="<%=basePath%>szy.jsp?page="+page;
                form3.submit();
            }else{
                form3.action="<%=basePath%>szy.jsp?page="+page;
                form3.submit();
    ...
    <div class="rmain">
        ...
        <div class="rlist">
            <table width="100%" border="0" align="left" cellpadding="4" cellspacing="0" class="rtable">
                <tbody>
                <tr class="tr1">
                    <td class="rldate daslist">实验标题</td>
                    <td class="rldate daslist">实验要求</td>
                ...
                <%
                    ...
                    int cou = cb.getMessageCount("select count(*) from zy where bt like '%"+word+"%'");//得到信息总数
                    ...
                    List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from zy where bt like '%"+word+"%' order by id desc",7);
                ...
                %>
                <tr class="tr<%=(i)%2%>">
                    <td class="rltitle dotlist"><a href="<%=basePath %>zyinfo.jsp?id=<%=pagelist2.get(0).toString()%>"><%=pagelist2.get(1).toString() %></a></td>
