...
function top2(){
   	form3.action="<%=basePath%>admin/member/sinfo.jsp?page=1";
    form3.submit();
}
function last2(){
	...
	form3.action="<%=basePath%>admin/member/sinfo.jsp?page="+form3.pageCount.value;
    	form3.submit();
	}
}
function pre2(){
  ...
  if(page<=1){
    ...
    form3.action="<%=basePath%>admin/member/sinfo.jsp?page="+(page-1);
    form3.submit();
  }
}
...
function bjump2(){
	...
	if(form3.busjump.value>pageCount){//如果跳转文本框中输入的页数超过最后一页的数，则跳到最后一页
	  ...
	  form3.submit();
		...
		form3.action="<%=basePath%>admin/member/sinfo.jsp?page="+pageCount;
		form3.submit();
	}
}
...
var page=parseInt(form3.busjump.value);
   if(page==0){
      ...
      form3.action="<%=basePath%>admin/member/sinfo.jsp?page="+page;
      form3.submit();
   }else{
      form3.action="<%=basePath%>admin/member/sinfo.jsp?page="+page;
      form3.submit();
...
<div class="title_right"><strong>注册用户管理</strong></div>  
...
<%String word= (request.getParameter("word"));
	...
	int cou = cb.getMessageCount("select count(*) from member where username like '%"+word+"%'");//得到信息总数			        
	...
	List pagelist1 = cb.getMessage(Integer.parseInt(page1),"select * from member where username like '%"+word+"%' order by id desc",12);
...
%>
       <tr align="center">
           ...
           <td nowrap="nowrap"><img src="<%=basePath+pagelist2.get(11).toString() %>" width="100" height="150" border="0" ></td>
           <td nowrap="nowrap">
               <a href="<%=basePath%>member/upifuse/<%=pagelist2.get(0).toString()%>"><%=pagelist2.get(9).toString() %></a>
               <a href="<%=basePath%>member/del/<%=pagelist2.get(0).toString()%>">删除</a>
           </td>
       </tr>
<% } %>
       <tr align="center">
         <td nowrap="nowrap" colspan="11"><input type="hidden" name="word" value="<%=word%>" />
